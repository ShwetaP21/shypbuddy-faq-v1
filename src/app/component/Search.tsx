...
export default function Search({ setSearchResults, searchClearOrNot, setClearSearchBar }) {

  console.log("line 119", searchClearOrNot);
// export default function Search({ setSearchResults }) {

  // const [searchTerm, setSearchTerm] = useState('');
  // const [query, setQuery] = useState<(FAQ | ContactInfoObjItem | ShypBuddyTermsObjItem | GlossaryObjItem | SubAnswer)[]>([]);
  ...
  // const handleSearch = () => {
    ...
    const handleSearch = () => {
      ...
      // const fuse = new Fuse([...GlossaryObj, ...ContactInfoObj, ...ShypBuddyTermObj], fuseOptions);
      const fuse = new Fuse(
        [
          ...CreditSummaryObj,
  ...
  //   setSearchTerm("");
  //   setSearchResults([]);
  //   // Additional logic to reset Search component state if needed
  // };
    return (
      <>
        <form  className="max-w-xl mx-auto p-2 md:w-4/5 w-[87%]   mt-5 justify-center text-center">
          ...
          <div className="relative">
            ...
            {/* <input type="search" autoComplete="off" id="default-search" className="block w-full p-4 ps-10 text-sm text-gray-900   rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 " placeholder="Search Mockups, Logos..." required onChange={handleInputChange} /> */}
            
            <input ref={inputRef} type="search" autoComplete="off" id="default-search" className="block w-full p-4 ps-10 text-sm text-gray-900   rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 " placeholder="What are you looking for?" required onChange={handleInputChange} />
            <button onClick={handleSearchButtonClick} className="absolute right-0 top-0 mt-1.5 mr-3 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Search</button>
            </div>
            {/* {searchClearOrNot && clearSearch()} Clear search on navigation */}
        </form>
      </>
    );
  }
